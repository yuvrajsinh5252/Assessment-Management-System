...
async function generate(req, res) {
  try {
    ...
    if (!assessmentData) {
      return res
        .status(404)
        .json({ message: `No assessment found for session_id ${sessionId}` });
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to generate report", details: error.message });
  }
}
...
async function listSessions(req, res) {
  try {
    const sessions = assessments.map((entry) => {
      ...
      const latestReport = getLatestReportMeta(
        entry.session_id,
        entry.assessment_id
      );
    ...
    res
      .status(500)
      .json({ message: "Unable to load sessions", details: error.message });
  }
}
